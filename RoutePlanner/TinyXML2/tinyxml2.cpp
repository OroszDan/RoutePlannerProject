...
*/
void XMLUtil::ToStr( float_t v, char* buffer, int bufferSize )
{
...
bool XMLUtil::Tofloat_t( const char* str, float_t* value )
{
...
XMLError XMLAttribute::Queryfloat_tValue( float_t* value ) const
{
    if ( XMLUtil::Tofloat_t( Value(), value )) {
        return XML_SUCCESS;
    }
    return XML_WRONG_ATTRIBUTE_TYPE;
}
...
void XMLAttribute::SetAttribute( float_t v )
{
    char buf[BUF_SIZE];
    XMLUtil::ToStr( v, buf, BUF_SIZE );
    _value.SetStr( buf );
...
float_t XMLElement::float_tAttribute(const char* name, float_t defaultValue) const
{
	float_t f = defaultValue;
	Queryfloat_tAttribute(name, &f);
	return f;
}
...
void XMLElement::SetText( float_t v )
{
    char buf[BUF_SIZE];
    XMLUtil::ToStr( v, buf, BUF_SIZE );
    SetText( buf );
...
XMLError XMLElement::Queryfloat_tText( float_t* fval ) const
{
    if ( FirstChild() && FirstChild()->ToText() ) {
        const char* t = FirstChild()->Value();
        if ( XMLUtil::Tofloat_t( t, fval ) ) {
            return XML_SUCCESS;
        }
        return XML_CAN_NOT_CONVERT_TEXT;
    }
...
float_t XMLElement::float_tText(float_t defaultValue) const
{
	float_t f = defaultValue;
	Queryfloat_tText(&f);
	return f;
...
void XMLPrinter::PushText( float_t value )
{
    char buf[BUF_SIZE];
    XMLUtil::ToStr( value, buf, BUF_SIZE );
    PushText( buf, false );
